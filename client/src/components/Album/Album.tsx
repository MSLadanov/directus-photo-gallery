...
const Album: React.FC = observer(() => {
  ...
  let photoId =
    locationArray.length > 2 ? locationArray[locationArray.length - 1] : "";
  useEffect(() => {
  ...
  useEffect(() => {
    if (!isLoading && albums.length > 0 && albumId) {
      let album = toJS(albums).find(
        (item: Album) => Number(item.id) === Number(albumId)
      );
      if (!album) {
  ...
  useEffect(() => {
    if (!isLoading && photos.length > 0 && photoId) {
      const photo = photos.find(
        (item: Photo) => Number(item.id) === Number(photoId)
      );
      if (photo) {
        ...
        <>
          ...
          <GridContainer>
            {paginatedPhotos.map((photo: Photo) => (
              ...
              >
                <PhotoImage
                  src={`/directus/assets/${photo.image}?fit=cover&height=150&quality=75`}
                />
